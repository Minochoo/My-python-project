...
def send_spam(uid):
    message = (
        ...
        f" Wrong ID ..\n"
        f" Please Check Again\n"
        f" DEV BOT @MR3SKR @MOHP4 @speidy_stopixx\n"
        f"-----------------------------------"
    )
    ...
    print(info_response)
    if info_response['status'] == "ok":
        requests.get(f"http://147.93.123.53:50063/request?api_key=22&uid={uid}")
        message = (
...
def remove_user(uid):
	requests.get(f"http://164.92.134.31:5111/remove/{uid}?key=99")
...
class FF_CLIENT(threading.Thread):
    ...
    def GenResponsMsg(self, Msg, Enc_Id):
        ...
        nested_details.field2 = int(get_random_avatar())
    ...
    def connect(self, tok, host, port, packet, key, iv):
		...
		restart_program()
            ...
            if senthi == True:
                ...
                clients.send(
                        self.GenResponsMsg(
                            f"""[C][B]What\'s Up {nameinv} Ramadan Mubarak!

How Are You Doing? How Is Ramadan?

Iam [ff000000]ＰＡＲＡㅤＨＥＸᴮᴼᵀ[ffffff] And Im Here To Serve You.

Send [00ff00]/help [ffffff]So You Can Get To Know My Commands!.

Bot Made By [00ff00]ＰＡＲＡㅤＨＥＸᵀᴱᴬᴹ.""", idinv
                        )
                )
                senthi = False
...
Only Contact [00ff00]ＰＡＲＡㅤＨＥＸᵀᴱᴬᴹ![ffffff]

@mohp4 , @MR3SKR , @zitado_km , @sidexoo

Also, You Can Send [00ff00]/help [ffffff]So You Can Get To Know My Commands!.

Bot Made By [00ff00]ＰＡＲＡㅤＨＥＸᵀᴱᴬᴹ.""", uid
                    )
                )
	            ...
	            if "1200" in data.hex()[0:4] and b"/help" in data:
                	...
                	if time['status'] == "bad":
                		remove_user(uid)
                	     ...
                	     clients.send(
		                    self.GenResponsMsg(
		                        f"""[cُ][bَ][FFB300]Hi {user_name} !\nWelcome To FHX BOT!
	
	
	[cُ][bَ][FFB300] Commands!
	
	{lines}
	
	[C][B] [FFD700]PLAYER INFO
	
	[C][B] [FF6347] ✨/info <id>
	
	{generate_random_color()+lines}
	
	[C][B] [FFD700]TO ADD LIKES
	
	[C][B] [FF6347] ✨/likes <id>
	
	{generate_random_color()+lines}
	
	[C][B] [FFD700] TO CHECK BAN ACCOUNT
	
	[C][B] [FF6347] ✨/check <id>
	
	{generate_random_color()+lines}
	
	[C][B] [FFD700] ✨ TO USE CHAT GPT 
	
	[C][B] [FF6347] ✨/ai <The word>
	
	{generate_random_color()+lines}
	
	[C][B] [FFD700]TO SPAM USE
	
	[C][B] [FF6347] ✨/spam <id>
	
	{generate_random_color()+lines}
	
	[C][B] [FFD700]TO VIST USE 
	
	[C][B] [FF6347] ✨/vist <id>
	
	{generate_random_color()+lines}
	
	[cُ][bَ][FF0000]Telegram -> @Fares_hex""", uid
		                    )
		                )
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id, password)
        client_thread.start()
    except Exception as e:
